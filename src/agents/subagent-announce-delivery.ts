...
async function resolveActiveWakeWithRetries(
  ...
  let outcome = await resolveQueueEmbeddedPiMessageOutcome(
    sessionId,
    message,
    currentOptions,
  );
  ...
  for (;;) {
    ...
    if (
      ...
      outcome = await resolveQueueEmbeddedPiMessageOutcome(
        sessionId,
        message,
        currentOptions,
      );
    ...
    if (
      ...
      await waitForAnnounceRetryDelay(
        compactionRetryDelaysMs[compactionRetryIndex] ?? 0,
        signal,
      );
      if (signal?.aborted) {
        break;
