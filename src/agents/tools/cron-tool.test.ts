...
describe("cron tool", () => {
  ...
  describe("wake routing", () => {
    ...
    // main lane (closes ClawSweeper P1 on PR #83738).
    ...
    it("derives agentId from an explicit cross-agent sessionKey instead of the caller's agentId", async () => {
      // Regression for ClawSweeper P1: a caller in agent-123 explicitly waking
      // an agent-456 session must NOT have agent-123's agentId paired with
      // agent-456's sessionKey — that would canonicalize back to agent-123's
      // main lane on the gateway side.
      const tool = createTestCronTool({
        agentSessionKey: "agent:agent-123:telegram:direct:channing",
      });
