...
const mockMaintainConfigBackups = vi.hoisted(() =>
  vi.fn(async (_configPath: string, _fsPromises: unknown) => {}),
);
...
vi.mock("./backup-rotation.js", async (importOriginal) => {
  ...
  return {
    ...
    maintainConfigBackups: (
      configPath: string,
      fsPromises: Parameters<typeof actual.maintainConfigBackups>[1],
    ) => mockMaintainConfigBackups(configPath, fsPromises),
  };
});
