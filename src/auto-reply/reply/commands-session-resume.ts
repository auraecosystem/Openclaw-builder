...
async function restoreSessionTranscript(
    ...
    if (!relative.startsWith("..") && !path.isAbsolute(relative)) {
      if (entry.sessionFile) {
        ...
        if (rawPath && !candidates.includes(rawPath) && fs.existsSync(rawPath)) {
          const rawRelative = path.relative(sessionsDirResolved, rawPath);
          if (!rawRelative.startsWith("..") && !path.isAbsolute(rawRelative)) {
            return rawPath;
          }
          const targetPath = path.join(sessionsDirResolved, path.basename(rawPath));
          try {
            await fsPromises.mkdir(sessionsDirResolved, { recursive: true });
            await fsPromises.copyFile(rawPath, targetPath);
            return targetPath;
          } catch {
            // fall through to normalized candidate
          }
        }
      }
      return candidate;
  ...
  if (entry.sessionFile) {
    ...
    if (rawPath && !candidates.includes(rawPath) && fs.existsSync(rawPath)) {
      ...
      const sessionsDirResolved = path.resolve(sessionsDir);
      const relative = path.relative(sessionsDirResolved, rawPath);
      if (!relative.startsWith("..") && !path.isAbsolute(relative)) {
        return rawPath;
      }
      const targetPath = path.join(sessionsDirResolved, path.basename(rawPath));
