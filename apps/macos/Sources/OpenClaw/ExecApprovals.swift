...
enum ExecApprovalsStore {
        ...
        dropInvalid: Bool) -> [ExecDenylistEntry]
    {
        ...
        for entry in entries {
            ...
            if pattern.isEmpty {
                if !dropInvalid {
                    normalized.append(ExecDenylistEntry(id: entry.id, pattern: entry.pattern, flags: entry.flags))
                }
                continue
            }
    ...
    private static func migrateUneditedManagedDefaultDenylistEntries(_ entries: inout [ExecDenylistEntry]) {
