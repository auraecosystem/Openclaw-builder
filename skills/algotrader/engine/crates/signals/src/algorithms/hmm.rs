...
impl HmmModel {
    /// Construct an HMM model from SignalParams fields.
    ///
    /// Converts raw probability values to log-space for Viterbi decoding.
    pub fn from_params(params: &engine_types::SignalParams) -> Self {
        let log_trans = [
            [ln(params.hmm_transition[0][0]), ln(params.hmm_transition[0][1]), ln(params.hmm_transition[0][2])],
            [ln(params.hmm_transition[1][0]), ln(params.hmm_transition[1][1]), ln(params.hmm_transition[1][2])],
            [ln(params.hmm_transition[2][0]), ln(params.hmm_transition[2][1]), ln(params.hmm_transition[2][2])],
        ];
        Self {
            n_states: params.hmm_n_states,
            log_trans,
            emission_mean: params.hmm_emission_mean,
            emission_var: params.hmm_emission_var,
            log_init: [
                ln(params.hmm_init_probs[0]),
                ln(params.hmm_init_probs[1]),
                ln(params.hmm_init_probs[2]),
            ],
        }
    }
}
