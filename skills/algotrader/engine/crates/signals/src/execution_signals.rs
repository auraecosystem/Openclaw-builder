...
pub fn execution_signals(
    ...
    let trend_factor = if kalman_trend > 0.0 {
        // Scale up for strong trends (capped by kalman_trend_up_cap).
        1.0 + (kalman_trend.abs() * params.kalman_trend_amplifier).min(params.kalman_trend_up_cap)
    } else {
        // Scale down for weakening/reversing trends (floored by kalman_trend_down_cap).
        (1.0 - (kalman_trend.abs() * params.kalman_trend_amplifier).min(params.kalman_trend_down_cap)).max(params.kalman_trend_down_cap)
    };
    ...
    let conformal_size_scale = match conformal_width {
        Some(w) if w > 0.0 && w.is_finite() => (1.0 / w).clamp(params.conformal_size_floor, params.conformal_size_ceil),
        _ => 1.0,
    };
