...
pub struct BocpdState {
    ...
    // -- Sufficient statistics per run length (Normal-Inverse-Gamma) --
    // Prior hyperparameters for the conjugate update:
...
impl BocpdState {
    /// Create a new BOCPD state with the given truncation limit.
    ...
    fn predictive(&self, r: usize, x: f32) -> f32 {
        ...
        let kappa_n = 1.0 + n;
        let mu_n = s / kappa_n; // (kappa0*mu0 + S) / kappa_n, mu0=0
        let alpha_n = 1.0 + n * 0.5;
        // beta_n = 1 + 0.5*(Q - S^2/kappa_n)
        let beta_n = (1.0 + 0.5 * (q - s * s / kappa_n)).max(1e-10);
