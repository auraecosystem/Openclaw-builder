...
function getDenylistFlags(entry: ExecApprovalsDenylistEntry): string | undefined {
  return typeof entry === "string" ? undefined : entry.flags;
}

function isValidDenylistFlags(flags: string | undefined): boolean {
  return flags == null || /^[imu]*$/u.test(flags.trim());
}
...
function renderDenylistEntry(
  ...
  const flags = getDenylistFlags(entry);
