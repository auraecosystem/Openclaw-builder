...
describe("session startup catch-up", () => {
  ...
  it("retries transient newline counting during session delta updates", async () => {
    const session = await writeSessionFile("delta.jsonl");
    const harness = new SessionStartupCatchupHarness([]);
    let attempts = 0;
  ...
  it("retries transient session transcript open failures during delta updates", async () => {
    const session = await writeSessionFile("delta-open.jsonl");
    const harness = new SessionStartupCatchupHarness([]);
    let attempts = 0;
    const sessionBuffer = await fs.readFile(session.filePath);
    const openSpy = vi
