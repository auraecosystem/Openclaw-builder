...
vi.mock("openclaw/plugin-sdk/channel-streaming", () => ({
  ...
  resolveChannelProgressDraftLabel: (params: {
    entry?: { streaming?: { progress?: { label?: string | false } } };
    seed?: { label?: string };
  }) => {
    const configured = params.entry?.streaming?.progress?.label;
    return configured === false ? undefined : (configured ?? params.seed?.label ?? "Thinking");
  },
  resolveChannelProgressDraftConfig: (entry?: {
    streaming?: { progress?: { label?: string | false } };
