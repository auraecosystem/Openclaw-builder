...
type EdenAiPricingShape = {
  ...
  input_cache_read_cost_per_token?: number | string;
  input_cache_write_cost_per_token?: number | string;
};
...
function normalizeCost(pricing?: EdenAiPricingShape): ModelDefinitionConfig["cost"] {
  return {
    ...
    cacheRead: toPerMillionCost(pricing?.input_cache_read_cost_per_token),
    cacheWrite: toPerMillionCost(pricing?.input_cache_write_cost_per_token),
  };
}
