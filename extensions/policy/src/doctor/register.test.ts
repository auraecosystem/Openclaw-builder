...
describe("registerPolicyDoctorChecks", () => {
  ...
  it.each([
    ...
    [
      "scopes empty sandboxBackends",
      { scopes: { docker: { sandboxBackends: [] } } },
      "oc://policy.jsonc/scopes/docker/sandboxBackends",
    ],
    [
      "scopes duplicate normalized sandboxBackends",
      { scopes: { docker: { sandboxBackends: ["Docker", "docker"] } } },
      "oc://policy.jsonc/scopes/docker/sandboxBackends/#1",
    ],
    [
      "scopes backend selector with tools",
      {
        scopes: {
          docker: {
      ...
      "oc://policy.jsonc/scopes/docker/tools",
    ],
    [
  ...
  it("applies backend-scoped sandbox claims only to matching sandbox backends", async () => {
    const configPath = join(workspaceDir, "openclaw.jsonc");
    const cfg = {
      ...cfgWithPolicy(),
      agents: {
        defaults: {
          sandbox: {
            mode: "all",
            backend: "docker",
            docker: { binds: ["/workspace:/workspace:rw"] },
          },
        },
        list: [
          {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ...
        ocPath: "oc://openclaw.config/agents/defaults/sandbox/docker/binds/#0",
        requirement:
          "oc://policy.jsonc/scopes/dockerSandbox/sandbox/containers/requireReadOnlyMounts",
      }),
    ]);
  });

  it("combines agent and backend selectors for sandbox claims", async () => {
    const configPath = join(workspaceDir, "openclaw.jsonc");
    const cfg = {
      ...cfgWithPolicy(),
      agents: {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/sandbox-container-mount-mode-required",
        ocPath: "oc://openclaw.config/agents/list/#0/sandbox/docker/binds/#0",
        requirement:
          "oc://policy.jsonc/scopes/runnerDocker/sandbox/containers/requireReadOnlyMounts",
      }),
    ]);
  });
