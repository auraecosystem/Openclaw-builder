...
export type PolicyScopeSelectorKind = "agentIds" | "sandboxBackends";
...
export const POLICY_RULE_METADATA = [
  ...
  },
  {
    ...
    emptyList: "disabled",
    scopeSelectors: ["agentIds", "sandboxBackends"],
  },
  {
...
function scopedPolicyShapeFinding(
  ...
  if (!isRecord(value)) {
    return policyShapeFinding(
      ...
      `Fix ${params.policyPath} so scopes maps scope names to policy overlays with selectors such as agentIds or sandboxBackends.`,
    );
  }
  for (const [scopeName, overlay] of Object.entries(value)) {
    ...
    const hasSandboxBackends = overlay.sandboxBackends !== undefined;
    if (!hasAgentIds && !hasSandboxBackends) {
      return policyShapeFinding(
        params.policyPath,
        `oc://${params.policyDocName}/${targetPrefix}`,
        `${params.policyPath} scopes.${scopeName} must include at least one selector.`,
        `Add agentIds, sandboxBackends, or both to the named policy scope.`,
      );
    }
    if (hasAgentIds) {
    ...
    if (hasSandboxBackends) {
      const sandboxBackendsFinding = policyStringArrayPropertyShapeFinding(
        overlay.sandboxBackends,
        {
          policyDocName: params.policyDocName,
          policyPath: params.policyPath,
          property: `scopes.${scopeName}.sandboxBackends`,
          target: `${targetPrefix}/sandboxBackends`,
          valueName: "sandbox backend",
        },
      );
      if (sandboxBackendsFinding !== undefined) {
        return sandboxBackendsFinding;
      }
    }
    if (Array.isArray(overlay.sandboxBackends) && overlay.sandboxBackends.length === 0) {
      return policyShapeFinding(
        params.policyPath,
        `oc://${params.policyDocName}/${targetPrefix}/sandboxBackends`,
        `${params.policyPath} scopes.${scopeName}.sandboxBackends must include at least one sandbox backend.`,
        `Add one or more sandbox backend ids to ${params.policyPath} scopes.${scopeName}.sandboxBackends.`,
      );
    }
    if (Array.isArray(overlay.sandboxBackends)) {
      const seen = new Map<string, number>();
      for (const [index, backend] of overlay.sandboxBackends.entries()) {
        ...
        if (previous !== undefined) {
          return policyShapeFinding(
            params.policyPath,
            `oc://${params.policyDocName}/${targetPrefix}/sandboxBackends/#${index}`,
            `${params.policyPath} scopes.${scopeName}.sandboxBackends[${index}] duplicates sandboxBackends[${previous}] after normalization.`,
            `List each sandbox backend only once per named policy scope.`,
          );
        }
        seen.set(normalized, index);
      }
    ...
    if (unsupportedKey !== undefined) {
      return policyShapeFinding(
        ...
        `${params.policyPath} scopes.${scopeName}.${unsupportedKey} is not supported by the configured scope selectors.`,
        `Use only supported selectors with agents.workspace, tools, or sandbox in this policy scope.`,
      );
    }
    if (!hasAgentIds && (overlay.agents !== undefined || overlay.tools !== undefined)) {
      const section = overlay.agents !== undefined ? "agents" : "tools";
      return policyShapeFinding(
        params.policyPath,
        `oc://${params.policyDocName}/${targetPrefix}/${section}`,
        `${params.policyPath} scopes.${scopeName}.${section} requires the agentIds selector.`,
        `Add agentIds to this named policy scope or move ${section} rules to an agent-scoped policy.`,
      );
    }
...
function sandboxPostureFindings(
  ...
  for (const target of sandboxScopedPolicyTargets(policy)) {
    ...
    findings.push(
      ...sandboxPostureFindingsForRule(
        ...
        evidence,
        (entry) => scopedSandboxTargetMatches(entry, target, evidence.sandboxPosture ?? []),
      ),
    );
  }
  return findings;
...
type SandboxScopedPolicyTarget = {
...
function scopedSandboxTargetMatches(
  entry: PolicySandboxPostureEvidence,
  target: SandboxScopedPolicyTarget,
  entries: readonly PolicySandboxPostureEvidence[],
): boolean {
  if (target.agentId !== undefined && !scopedSandboxAgentMatches(entry, target.agentId, entries)) {
    return false;
  }
  if (
    target.sandboxBackend !== undefined &&
    !scopedSandboxBackendMatches(entry, target.sandboxBackend, entries, target.agentId)
  ) {
    return false;
  }
  return true;
...
function scopedSandboxBackendMatches(
  entry: PolicySandboxPostureEvidence,
  sandboxBackend: string,
  entries: readonly PolicySandboxPostureEvidence[],
  policyAgentId?: string,
): boolean {
  const backend = sandboxBackendForEntry(entry, entries, policyAgentId);
  return (
    typeof backend?.value === "string" &&
    backend.value.toLowerCase() === sandboxBackend.toLowerCase()
  );
}

function sandboxBackendForEntry(
  entry: PolicySandboxPostureEvidence,
  entries: readonly PolicySandboxPostureEvidence[],
  policyAgentId?: string,
): PolicySandboxPostureEvidence | undefined {
  if (entry.kind === "backend" && policyAgentId === undefined) {
    return entry;
  }
  if (policyAgentId !== undefined) {
    return (
      entries.find(
        (candidate) =>
          candidate.scope === "agent" &&
      ...
      entries.find((candidate) => candidate.scope === "defaults" && candidate.kind === "backend")
    );
  }
  if (entry.scope === "agent") {
    return entries.find(
      (candidate) =>
        candidate.scope === "agent" &&
        candidate.kind === "backend" &&
        scopedAgentIdMatches(candidate.agentId, entry.agentId ?? ""),
    );
  }
  return entries.find(
    (candidate) => candidate.scope === "defaults" && candidate.kind === "backend",
  );
}
...
function sandboxScopedPolicyTargets(policy: unknown): readonly SandboxScopedPolicyTarget[] {
  const targets: SandboxScopedPolicyTarget[] = [];
  for (const [scopeName, overlay] of agentScopedPolicyOverlays(policy)) {
    if (!isRecord(overlay.sandbox)) {
      continue;
    }
    const agentIds = Array.isArray(overlay.agentIds)
      ? overlay.agentIds
          .filter((entry): entry is string => typeof entry === "string" && entry.trim() !== "")
          .map((entry) => normalizeAgentId(entry))
      : [undefined];
    const sandboxBackends = Array.isArray(overlay.sandboxBackends)
      ? overlay.sandboxBackends
          .filter((entry): entry is string => typeof entry === "string" && entry.trim() !== "")
          .map((entry) => entry.trim().toLowerCase())
      : [undefined];
    for (const agentId of agentIds) {
      for (const sandboxBackend of sandboxBackends) {
        targets.push({ scopeName, agentId, sandboxBackend, overlay });
      }
    }
  }
  return targets;
    ...
    for (const agentId of agentIds) {
      for (const sandboxBackend of sandboxBackends) {
        for (const field of fields) {
          ...
          const backendKey = field.metadata.scopeSelectors?.includes("sandboxBackends")
            ? sandboxBackend
            : "";
          const key = `${agentId}\0${backendKey}\0${field.fieldPath}`;
...
function scopedAgentPolicyFields(
  ...
  return POLICY_RULE_METADATA.filter((rule) => {
    ...
    if (rule.policyPath[0] !== "sandbox") {
      return selectors?.includes("agentIds") && overlay.agentIds !== undefined;
    }
    return (
      (selectors?.includes("agentIds") && overlay.agentIds !== undefined) ||
      (selectors?.includes("sandboxBackends") && overlay.sandboxBackends !== undefined)
    );
  })
