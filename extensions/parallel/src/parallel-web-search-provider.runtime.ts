...
function missingObjectivePayload() {
  return {
    error: "missing_objective",
    message:
      "objective is required: a natural-language description of the underlying question or goal. Used together with search_queries to focus results.",
    docs: "https://docs.openclaw.ai/tools/parallel-search",
  };
}
...
export async function executeParallelWebSearchProviderTool(
  ...
  if (!objective) {
    return missingObjectivePayload();
  }
  const searchQueries = normalizeParallelSearchQueries(
    readStringArrayParam(args, "search_queries"),
  );
