...
[Eden AI](https://www.edenai.co) is a single API that routes to many model vendors (OpenAI, Anthropic, Google, Mistral, Cohere, and more). One key, one base URL, many models. OpenClaw includes a bundled Eden AI provider plugin.
...
1. **Stale session transcript from a previously failed model.** Wipe the sessions and retry:
