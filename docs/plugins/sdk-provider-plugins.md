...
<Steps>
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      ...
      - `resolveThinkingProfile(ctx)` receives the selected `provider`, `modelId`, optional merged `reasoning` catalog hint, and optional merged model `compat`/`params` facts. Use `compat` and `params` only to select the provider's thinking UI/profile; do not mutate them or rely on them containing secrets.
