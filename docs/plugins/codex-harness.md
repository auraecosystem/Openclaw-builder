...
For local stdio app-server launches, OpenClaw checks the managed per-agent
`CODEX_HOME` before startup and rotates `logs_2.sqlite`, `logs_2.sqlite-wal`,
and `logs_2.sqlite-shm` when their combined size exceeds the retention cap. The
