...
<AccordionGroup>
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    and optional merged model `compat`/`params` facts. Provider plugins can use
    those catalog facts to expose a model-specific profile only when the
    configured request contract supports it.
