...
For webhook ingress, it warns when `hooks.token` reuses the Gateway token, when `hooks.token` is short, when `hooks.path="/"`, when `hooks.defaultSessionKey` is unset, when `hooks.allowedAgentIds` is unrestricted, and when request `sessionKey` overrides are enabled without `hooks.allowedSessionKeyPrefixes`; it emits an INFO-level acknowledgement when request `sessionKey` overrides are enabled with prefixes constrained.
