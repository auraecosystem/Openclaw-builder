...
- `progress` mode keeps tool progress in an editable status draft, clears that draft at completion, and sends the final answer through normal delivery. Telegram can keep that progress log visible with `streaming.progress.keepLog: true`.
...
Telegram-only progress log retention:
