...
- `searchId`, `warnings`, and `usage` from Parallel are passed through when
  present
- `count` is only forwarded to Parallel when the caller (or the top-level
  `tools.web.search.maxResults` setting) supplies one. Parallel best practices
  recommend omitting `max_results` unless there is a product need
