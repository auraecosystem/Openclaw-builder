...
- Profile hooks receive merged catalog facts when available, including `reasoning`, `compat.thinkingFormat`, `compat.supportedReasoningEfforts`, and model `params`. Use those facts to expose binary or custom profiles only when the configured request contract supports the matching payload.
