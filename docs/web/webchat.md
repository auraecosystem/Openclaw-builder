...
- `chat.message.get` uses the same transcript branch and display projection rules as `chat.history`, but targets one transcript entry by `messageId` and returns an honest unavailable reason when the full content can no longer be returned.
