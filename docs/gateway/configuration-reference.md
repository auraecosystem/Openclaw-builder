...
- `modelPreflight`: local model-provider preflight controls for isolated cron agent turns. Increase `maxAttempts`, `retryDelayMs`, or `timeoutMs` when a sleeping Ollama/vLLM/LM Studio host needs a short wake-up window before cron marks the run skipped. The worst-case window (`timeoutMs * maxAttempts + retryDelayMs * (maxAttempts - 1)`) must stay at or below 55s so preflight remains below cron's isolated-agent setup watchdog.
