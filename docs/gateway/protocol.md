...
<AccordionGroup>
  ...
  <Accordion title="Session control">
    ...
    - `chat.send` accepts one-turn `fastMode: "auto"` plus `fast_seconds` (`fastSeconds` alias) to test or override the automatic cutoff without editing persistent session/config state.
